...
#   Requires: bash, mv, rm, tr, type, curl/wget, sudo (if not root)
...
install_caddy()
{
	...
	caddy_asc="https://caddyserver.com/download/$caddy_os/$caddy_arch$caddy_arm/signature?plugins=$caddy_plugins"
	echo "$caddy_url"
	...
	if type -p curl >/dev/null 2>&1; then
		curl -fsSL "$caddy_url" -o "$dl"
		((gpg)) && curl -fsSL "$caddy_asc" -o "$dl.asc"
	elif type -p wget >/dev/null 2>&1; then
		wget --quiet "$caddy_url" -O "$dl"
		((gpg)) && wget --quiet "$caddy_asc" -O "$dl.asc"
	else
	...
	if ((gpg)); then
		...
		gpg --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys $caddy_pgp >/dev/null 2>&1
		if gpg -q --batch --verify "$dl.asc" "$dl" >/dev/null 2>&1; then
			rm -- "$dl.asc"
